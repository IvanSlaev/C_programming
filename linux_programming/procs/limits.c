...
void print_limits(void)
{
	...
	do_limit(RLIMIT_FSIZE, "RLIMIT_FSIZE", &rlim);
	do_limit(RLIMIT_DATA, "RLIMIT_DATA", &rlim);
...
void print_rusage(int who)
{
	...
	printf("ru_utime.tv_sec, ru_utime.tv_usec = %4d %4d (user time used)\n", (int)usage.ru_utime.tv_sec, (int)usage.ru_utime.tv_sec);
	printf("ru_stime.tv_sec, ru_stime.tv_usec = %4d %4d (system time used)\n", (int)usage.ru_stime.tv_sec, (int)usage.ru_stime.tv_sec);
	printf("ru_maxrss = %4ld (max resident se size)\n", usage.ru_maxrss);
	...
	printf("ru_msgsnd = %4ld (messages send)\n", usage.ru_msgsnd);
...
int main (int argc, char *argv[])
{
	...
	printf("\nThis is RLIMIT_CORE, before modification:");
	do_limit(RLIMIT_CORE, "RLIMIT_CORE", &rlim);
	rlim.rlim_cur = 8 * 1024 * 1024;
	printf("I forked off a child with pid: %d\n", (int)pid);
	
	setrlimit(RLIMIT_CORE, &rlim);
	printf("\nThis is RLIMIT_CORE, after modification:");
	do_limit(RLIMIT_CORE, "RLIMIT_CORE", &rlim);
